  ...
  ConvertStyle(){
    ...
    while(aa:=All.Item[A_Index-1]){
      ...
      for a,Text in StrSplit(aa.Text,Chr(127)){
        Pos:=LastPos:=1
        while(RegExMatch(Text,"Oim`n)^\s*(?(\w|[^\x00-\x7F])+):(.*)$",Found),Pos:=Found.Pos("Text")+Found.Len("Text")){
...
Class MainWindowClass{
  ...
  __New(){
    ...
    */
    ...
    if((node:=obj.Gui.SSN("//*[@type='Tracked Notes' and (@x+4<" x " and @x+@w+-4>" x ")and(@y<" y " and @y+@h>" y ")]"))&&((tnea.xx-4)||(tnea.yy-4))){
    ...
    */
    if(ri.top),ea:=XML.EA(ri.top){
      ...
      if(ea1.x=ea2.x&&ea1.x+ea1.w=ea2.x+ea2.w&&ea1.x!=""&&ea2.x!="")
        limits.top[ea1.yea2.y+ea2.h?ea1.y+ea1.h:ea2.y+ea2.h]:=1
      else{
        ...
        while(node:=xx.SSN("//*[@win=1]/descendant::*[@y=" y " and @x+@w=" x "]"),lea:=XML.EA(node))
          x:=lea.x,node.SetAttribute("top",1),limits.bottom[lea.y+lea.h]:=1,minx:=lea.xmaxx?lea.x+lea.w:maxx
        x:=ox
        while(node:=xx.SSN("//*[@win=1]/descendant::*[@y=" y " and @x=" x+w "]"),lea:=XML.EA(node))
          x:=lea.x,w:=lea.w,node.SetAttribute("top",1),limits.bottom[lea.y+lea.h]:=1,minx:=lea.xmaxx?lea.x+lea.w:maxx
      ...
      if(ea1.y=ea2.y&&ea1.y+ea1.h=ea2.y+ea2.h&&ea1.y!=""&&ea2.y!="")
        limits.left[ea1.xea2.x+ea2.w?ea1.x+ea1.w:ea2.x+ea2.w]:=1
      else{
        ...
        while(node:=xx.SSN("//*[@win=1]/descendant::*[@x=" x " and @y+@h=" y "]"),tea:=XML.EA(node))
          y:=tea.y,node.SetAttribute("left",1),limits.right[tea.x+tea.w]:=1,miny:=tea.ymaxy?tea.y+tea.h:maxy
        y:=oy,h:=oh
        while(node:=xx.SSN("//*[@win=1]/descendant::*[@x=" x " and @y=" y+h "]"),tea:=XML.EA(node))
          y:=tea.y,h:=tea.h,node.SetAttribute("left",1),limits.right[tea.x+tea.w]:=1,miny:=tea.ymaxy?tea.y+tea.h:maxy
    ...
    while(GetKeyState("LButton")){
      ...
      if(limits.left!=""&&limits.top!=""){
        if(!(m.ylimits.top&&m.x>limits.left&&m.xlimits.top))
            Continue
        if(!limits.top&&!limits.bottom)
          if(!(m.x>limits.left&&m.x0&&!ea.lp)
    ...
    if(Search.Open){
      while(RegExMatch(Text,Search.Open,Start)){
        if(!RegExMatch(Text,Search.Close,End))
          Break
        While((RegExMatch(Text,Search.Close,End))"),No:=ScanTextXML.SSN("//*")
    for c,d in Omni{
      for a,b in d{
          ...
          while(RegExMatch(Text,b.Regex,Found,Pos),Pos:=Found.Pos(0)+Found.Len(0)){
            ...
            if(b.Open){
              ...
              Loop
              {
                ...
                if(!OP||!CP)
                  Break
                if(CP'" Start "']")).length)?CEXML.Under(Deepest.item[Deepest.Length-1],"info",Atts):CEXML.Under(Node,"info",Atts),New:=No.AppendChild(Spam.CloneNode(0))
    ...
    */
    ...
    if(SettingsClass.Tabs.Edit_Replacements=tab){
      ...
      if(action~="MSIU|MSID"){
        ...
        tv:=TV_GetPrev(ea.tv)?TV_GetPrev(ea.tv):"First",new:=menus.Add("separator",{clean:"",tv:(tv:=TV_Add("",SSN(node.ParentNode,"@tv").text,tv))},,1),node.ParentNode.InsertBefore(new,node)
...
Clear_History(){
  History.XML.XML.LoadXML(""),sc:=CSC(),History.Add(CEXML.EA("//*[@sc='" sc.2357 "']"),sc,1),SetStatus("History Cleared",2)
}
Clear_Line_Status(){
  LineStatus.Clear()
}
...
Context(return=""){
  ...
  if(Pos>1){
    ...
    if(FirstEA.First){
      ...
      if(NewStartWidth?Max+20:Width,RegExReplace(Comment,"\R",,Count)
  if(Count=0){
    Total:=CommentChar
...
Extract(Main){
  ...
  while(RegExMatch(text,"iOm`nU)^\s*\x23Include\s*,?\s*(.*)(\s+;.*)?$",found,pos)),pos:=found.pos(1)+found.len(1){
    if(InStr(Found.2,";NoIndex"))
      Continue
    /*
      Make sure to take better care of 
      ...
      if(v.Options.Full_Tree){
        ...
        if(InStr(rel,"\")){
          if(!tv:=SSN(main,"descendant::folder[@path='" build "']/@tv").text){
            ...
            for a,b in Relative{
              build.=b "\"
              if(asc.2009?sc.2009:sc.2008,end:=sc.2008sc.2009?sc.2008:sc.2009,pos:=greater>start?greater-start:1
...
Fold_All(){
  ...
  current:=0
  while((current=0,fold:=sc.2618(fold)){
    lev:=sc.2223(fold)
    if(lev&0xff=level)
      sc.2237(fold,1)
    fold++
...
GetClassText(EA,SearchText,Type:="Class",ReturnClass:=0){
  ...
  if(RegExMatch(FileText,SearchText,found,IsObject(search)?Search.Pos(0):1)){
    ...
    while(RegExMatch(FileText,"OUm`n)((?^\s*\Q*/\E)|(?^\s*\Q/*\E)|(?^\s*}.*((\{)\s*(;.*)*)*)$)|((?.*\{)(\s+;.*|\t\w?\d?.*)*(\s*)*$)",found,Pos)),Pos:=found.Pos(0)+found.len(0){
      /*
        make a ? that deals with class opens
        have it make a not of the brace opens
        when it gets to that brace level, that's the text for that class.
        (?^\s*(class\s+((\w|[^\x00-\x7F])+))(\s*;.*\R){0,}\s*(\{))
        (?^[\s|}]*(class\s+((\w|[^\x00-\x7F])+))(\s*;.*\R){0,}\s*(\{))
...
GetFind(Text){
  Start:=InStr(Text,"(?^\s*\Q*/\E)|(?^\s*\Q/*\E)|(?^\s*}.*((\{)\s*(;.*)*)*)$)|((?.*\{)(\s+;.*)*(\s*)*$)",found,pos)),pos:=found.pos(0)+found.len(0){
      lastfound:=found
      if(!found.len)
        Break
      if(found.SkipOpen)
...
GetSearchRegex(FindSearch,Text,Replace:="Text"){
  if(RegExMatch(FindSearch,"OU)(\(\?\<" Replace "\>)",FF)){
    ...
    for a,b in StrSplit(FindSearch){
      if(A_Index=0?&GUID:"",DllCall("oleacc\ObjectFromLresult", "ptr", lResult,"ptr",CLSID,"ptr",0,"ptr*",pdoc),pweb:=ComObjQuery(pdoc,id:="{0002DF05-0000-0000-C000-000000000046}",id),ObjRelease(pdoc)
...
InputBox(Parent,Title,Prompt,Default=""){
  ...
  Width:=Max.MaxIndex()*Width,height:=(20*Count)+100,y:=((CPos:=sc.2165(0,sc.2008))=0)
    sc.2025(pos)
...
Omni_Search(start=""){
  ...
  while(ll:=List.Item[A_Index-1],b:=XML.EA(ll)){
    ...
    if(TypeInfo.Multiple){
      ...
      if(Item.Text)
        Search:=RegExReplace(Search,"i)\?\.\*","\Q"(Item.Text)"\E")
      Loop,% 1+Pre
        Pos:=RegExMatch(Text,Search,,Pos+1)
...
ParseJson(jsonStr){
  ...
  }SC.Language:="JScript",jsCode:="function arrangeForAhkTraversing(obj){if(obj instanceof Array){for(var i=0;i",FF)){
...
QF(x:=0){
  ...
  while(Start=0){
      MinMax.XML.LoadXML(""),Top:=MinMax.Add("list"),Last:=sc.2224(Parent,-1),MinMax.Under(Top,"sel",{min:sc.2167(Parent),max:sc.2167(Last)})
...
Right_Click_Menu_Editor(menu){
  ...
  New:=RCMXML.Add("separator",{clean:""},,1),Node.ParentNode.InsertBefore(New,Node)
  Update:=1
  Goto,RCME
  return
  RCMAS:
  ...
  if(A_GuiEvent~="Normal|I"|update){
    ...
    if(node:=RCMXML.SSN("//main[@name='" menu "']")){
      ...
      while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa)
        aa.SetAttribute("tv",TVRCM.Add("t1",aa.NodeName="menu"?ea.name:"",SSN(aa.ParentNode,"@tv").text))
...
Scan_Line(text:=""){
  while(b:=v.LineEdited.Pop()){
    ...
    */
    ...
    /*
      for c,d in {Breakpoint:"OUm`n)(\s+|^);\*\[(?.*)\]",Bookmark:"OUm`n)(\s+|^);#\[(?.*)\]"}{
        LastPos:=Pos:=1
...
ScanParent(Text,b){
  ...
  SP.XML.LoadXML(""),Bounds:=b.Bounds,LastOuterPos:=OuterPos:=1,Parents:=[]
...
SearchFor(b,Pos1){
  ...
  Loop
  {
    ...
    if(!OP&&!CP)
      Break
    if(CP&&OP){
      if(CP0)?(line)-half:0),sc.2613(NewLine)
}
